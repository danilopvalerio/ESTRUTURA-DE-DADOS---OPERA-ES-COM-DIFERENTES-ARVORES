...
public class ArvoreBinaria {
	...
	public ArvoreBinaria() { root=null; } // inicializa arvore
	...
	public void inserir(long v) {
		...
		if (root == null) {
			...
			while (true) {
				anterior = atual;
				if (v == atual.item) {
					// Se o valor já existe, não insere e retorna
					return;
				} else if (v < atual.item) { // ir para esquerda
					atual = atual.esq;
					if (atual == null) {
						anterior.esq = novo;
						return;
	...
	public int altura(No raiz) {
		...
		while (!fila.isEmpty()) {
			...
			while (tamanhoNivel > 0) {
				No no = fila.poll();

				// Adiciona os filhos na fila
				if (no.esq != null) fila.offer(no.esq);
				if (no.dir != null) fila.offer(no.dir);

				tamanhoNivel--;
			}

			// Incrementa a altura após processar todos os nós no nível atual
			altura++;
		}
